    ...
    while (unbind_event) {
        ...
        switch (determine()) {
            ...
            case 2:
                ...
                break;
            case 3:
                if (config.value("切角色1").toBool() && roles[1]) {
                    // switch_roles(1);
                    roles[1] = false;
                    return 1;
                }
                if (config.value("切角色2").toBool() && roles[2]) {
                    // switch_roles(2);
                    roles[2] = false;
                    return 1;
                }
                if (config.value("切角色3").toBool() && roles[3]) {
                    // switch_roles(3);
                    roles[3] = false;
                    return 1;
                }
                if (config.value("切角色4").toBool() && roles[4]) {
                    // switch_roles(4);
                    roles[4] = false;
                    return 1;
                }
                if (config.value("切角色5").toBool() && roles[5]) {
                    // switch_roles(5);
                    roles[5] = false;
                    return 1;
                }
